...
class ReactionDiffusion2D(eqx.Module):
    ...
    #    self, x
    #):
    #    react = self.NN.R(x).squeeze()
    #    #if react.shape[0] == 1:
    #    #    react = react.squeeze(0)

    #    phi = x[0:1]
    #    diffuse = self.NN.D(phi).squeeze()

    #    # print(react.shape, diffuse.shape)
...
class RDNOPrescribed(eqx.Module):
    #lifting: eqx.nn.Conv2d
    diffuseBlock : eqx.Module
    #projection: eqx.nn.Conv2d
    #bias : jax.Array
    nonlinear : Callable
    ...
    def __init__( 
        ...
        self.diffuseBlock = diffuseBlock
    ...
    def __call__(
        ...
        x = self.nonlinear(x)
        if len(x.shape) == 2:
            x = x[jnp.newaxis] # shape (in_channels, N_x, N_y)

        x = self.diffuseBlock(x) 
